...
```bash
npm run test:zodiac      # Run zodiac calculation tests (32 celebrity charts)
```

- **32 verified test cases** with Rodden Rating AA (birth certificate verified)
- **352 total calculations** per test run (11 points × 32 cases)
- **~93-96% accuracy rate** against verified astronomical data
- Tests all core points: Sun, Moon, Ascendant, plus 8 planets
- Test data from Astro-Databank, Astro-Seek, and Astrotheme
- Covers birth dates from 1879-2001 across multiple time zones
- Located in `src/lib/zodiac.test.ts`
- Tests include historical figures, celebrities, and musicians
